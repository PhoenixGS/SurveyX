...
class LatexTextBuilder:
    def __init__(self, init_tex_path: Union[str, Path]):
        # 确保 init_tex_path 是字符串或 Path 对象，并保存
        if isinstance(init_tex_path, Path):
            self.init_tex_path = str(init_tex_path)
        else:
            self.init_tex_path = init_tex_path
        self.tex = load_file_as_string(self.init_tex_path)
...
"""
    ...
    def run(
        ...
        """Convert mainbody to a standard latex paper format."""
        # 每次运行前重新初始化，避免累积内容
        self.tex = load_file_as_string(self.init_tex_path)
