...
from src.modules.latex_handler.latex_error_fixer import LaTeXErrorFixer
...
class LatexGenerator:
    ...
    def compile_single_survey(self):
        ...
        # 使用自动修复器进行编译
        error_fixer = LaTeXErrorFixer(self.task_id, latex_dir)
        compile_success = error_fixer.compile_and_fix()
        
        if not compile_success:
            logger.error(
                f"LaTeX 编译失败！PDF 文件未生成。"
                f"请检查编译日志: {latex_dir / 'compile.log'}"
            )
            # 尝试读取编译日志的最后几行以便调试
            try:
                with open(latex_dir / "compile.log", "r", encoding="utf-8") as f:
                    ...
                    if error_lines:
                        logger.error("编译日志中的关键错误:")
                        for line in error_lines[-10:]:  # 显示最后10个错误
                            logger.error(line.strip())
            except Exception as e:
                logger.error(f"读取编译日志失败: {e}")
            raise RuntimeError(
                f"LaTeX 编译失败，自动修复未成功。"
                f"请检查 {latex_dir / 'compile.log'} 获取详细信息。"
            )
        ...
        # 删除所有 .bbl 文件
        subprocess.run("rm *.bbl", shell=True, stderr=subprocess.DEVNULL)

        subprocess.run("rm neurips_2024.sty", shell=True)

        # 将生成的 survey.pdf 移动到上一级目录
        pdf_path = latex_dir / "survey.pdf"
        if pdf_path.exists():
