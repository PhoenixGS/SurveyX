...
class Outlines:
    ...
    def serial_no_to_single_outline(self, serial_no_raw: str) -> SingleOutline | None:
        ...
        Returns:
            SingleOutline: corresponding single outline, or None if not found.
        """
        try:
            if "." in serial_no_raw:
                ...
                # 检查主章节索引是否有效
                if primary_section_index < 1 or primary_section_index > len(self.sections):
                    logger.warning(
                        f"Primary section index {primary_section_index} out of range (1-{len(self.sections)}), "
                        f"serial_no_raw: {serial_no_raw}"
                    )
                    return None
                
                primary_section = self.sections[primary_section_index - 1]
                
                if secondary_section_index != "":
                    secondary_section_index = int(secondary_section_index)
                    # 检查子章节索引是否有效
                    if secondary_section_index < 1 or secondary_section_index > len(primary_section.sub):
                        logger.warning(
                            f"Secondary section index {secondary_section_index} out of range "
                            f"(1-{len(primary_section.sub)}) for primary section {primary_section_index}, "
                            f"serial_no_raw: {serial_no_raw}"
                        )
                        return None
                    return primary_section.sub[secondary_section_index - 1]
                else:
            ...
            logger.error(
                f"Error occurs: {e}, the serial_no_raw is {serial_no_raw}"
            )
            return None
